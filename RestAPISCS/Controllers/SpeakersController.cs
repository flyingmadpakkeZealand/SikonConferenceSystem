...
namespace RestAPISCS.Controllers
{
    ...
    {
        private ManageGenericWithLambda<Speaker>
            speakerManager = DataBases.Access<Speaker>(BaseNames.SikonDatabase, "Speaker");
        ...
        public bool Put(int id, [FromBody]Speaker speaker)
        {
            ...
            if (speakerOk)
            {
                userOk = userManager.Put(Extractables.ExtractUser(new User(speaker.Name, speaker.PhoneNumber,
                    speaker.Email, speaker.Password)), PrimaryKeys(phoneNumber));
            }
